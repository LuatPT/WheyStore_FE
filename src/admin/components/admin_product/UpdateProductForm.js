...
class UpdateProductForm extends React.Component {
  ...
  componentDidMount = () => {
    ...
    ShowUpdateProduct.getDetailProduct({ product_id: this.props.product_id });
  }
  updateProduct = (ele) => {
  ...
  render() {
    ...
    return (
      <tr key={key}>
        ...
        <td>
          <input type='text' defaultValue={product.product_name} ref={this.product_name} />
        </td>
        <td> <input type='text' defaultValue={product.category_id} ref={this.category_id} /></td>
        <td >
          <img src={product.product_img} alt="..." width="150px" height="150px" />
        </td>
        <td className="noteProduct">
          <textarea rows="6" defaultValue={product.product_note} ref={this.product_note} />
        </td>
        <td >
        ...
        <td >
          <input type='text'
            defaultValue={product.product_sale}
            ref={this.product_sale} />
        </td>
        <td className="endSaleProduct" >
          <input type='date'
            defaultValue={moment(product.product_endsale).format("YYYY-MM-DD")}
            ref={this.product_endsale} />
        </td>
        <td>
          <span className='btnEdit' onClick={() => this.updateProduct(product)}>
          ...
          <span className='btnDelete' onClick={() => this.deleteProduct(product)}>
